...
import { shuffled } from '@/common/lib';
import { appStarted } from '@/common/services';
...
export const ProductModel = atom(() => {
    const $categories = restore<string[]>(
        getAllProductsQuery.finished.success.map(({ result }) => mapCategoriesLabels(findCategories(result))),
        []
    );

    // 4 случайных товара со скидкой
    const $discountedItems = restore<Product[]>(
        getAllProductsQuery.finished.success.map(({ result }) =>
            shuffled((result as Product[]).filter((el) => Boolean(el.discount_price)).filter((el) => el.is_available)).slice(0, 4)
        ),
        []
    );
    ...
    const $availableProducts = restore<Product[]>(
        getAllProductsQuery.finished.success.map(({ result }) => (result as Product[]).filter((el) => el.is_available)),
        []
    );
    ...
    const $allCharacteristicsNames = $availableProducts.map((products: Product[]) =>
        products
            .map((product: Product) => product.characteristics)
            .flat(1)
